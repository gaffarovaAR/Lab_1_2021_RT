...
Ray Camera::genRay(float w, float h, float x, float y) const
{
   //прописать генерацию луча, выходящего из камеры
    float3 origin = eye;
    float3 ray_dir;
    // сейчас луч генерируется всегда через центр пикселя
    // чтобы повысить качество картинки, нужно выбирать случайное положение внутри пикселя
    ray_dir.x = x + 0.5f - (w / 2.0f);
    ray_dir.y = y + 0.5f - (h / 2.0f);
    ray_dir.z = -(w) / tan(fov / 2.0f);
    return Ray(origin, normalize(ray_dir));
}
